...
import { isOperationsPreview, previewConfig } from "../../../../src/lib/panel-preview";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const preview = isOperationsPreview() ? previewConfig() : null;
    const grants = preview ? [{ hash: preview.bootstrapHash, expiresAt: preview.expiresAt }] : bootstrapGrants;
    if (body.length > 1024 || !validBootstrap(JSON.parse(body).token, grants)) {
