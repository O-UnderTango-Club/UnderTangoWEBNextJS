...
- `activar_en` es un `timestamptz` independiente de `vencimiento`. Una fecha futura conserva frente y posición, pero excluye la acción de los tres cupos ejecutables. Al llegar el instante, el panel la considera elegible sin reordenar ni escribir durante la lectura.
...
Aplicar después `supabase/migrations/20260911022157_add_follow_up_activation_at.sql`. Mantiene v1/v2 para compatibilidad y agrega el contrato privado v3 con `activate_at`.
