...
const m=record('movements','sb-bc732d30-2ccf-4a32-aa99-7f62fc3f18e4',{currency:'ARS',amount:'40.00',date:'2026-01-05',status:'Confirmado',type:'Ingreso',notes:'Lote reconstruccion-2026-20260909.'});
...
const paid=record('movements','new-income',{currency:'ARS',amount:'80.00',date:'2026-09-09',status:'Confirmado',type:'Ingreso',notes:'Informe financiero 2026.',operations:['op']});
const unpaid=record('movements','future-income',{currency:'ARS',amount:'30.00',date:'2026-09-09',status:'Pendiente',type:'Ingreso',notes:'Informe financiero 2026.'});
const out=record('movements','expense',{currency:'ARS',amount:'10.00',date:'2026-09-09',status:'Confirmado',type:'Egreso',notes:'Informe financiero 2026.'});
const shares=Array.from({length:4},(_,i)=>record('obligations','fee'+i,{name:'Honorario artista '+i,currency:'ARS',originalAmount:'20.00',balance:'20.00',status:'Pendiente',type:'Por pagar',notes:'distribucion-honorarios-2026.',operations:['op']}));
r=read({...snapshot,movements:[paid,unpaid,out],obligations:[...snapshot.obligations,...shares],operations:[{id:'op',name:'Show de prueba'}]});
assert.equal(r.collectionMonths[8].amounts.ARS,80);assert.equal(r.collectionMonths[8].amounts.USD,null);assert.equal(r.collectionMonths[7].amounts.ARS,null);assert.equal(r.months[8].amounts.ARS,null);assert.equal(r.totals.ARS,100.1);
assert.equal(r.allocations.length,1);assert.equal(r.allocations[0].payments.length,4);assert.equal(r.allocations[0].received,80);assert.equal(r.allocations[0].committed,80);assert.equal(r.allocations[0].pending,80);assert.equal(r.allocations[0].unassigned,0);assert.equal(r.cash,null);
shares[0].fields[F.obligations.balance.id]=null;r=read({...snapshot,movements:[paid],obligations:shares});assert.equal(r.allocations[0].pending,null);
