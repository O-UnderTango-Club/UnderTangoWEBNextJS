...
try{
 ...
 const beforeComment=structuredClone(current.tasks[0].fields);
 await server.mutate({kind:'task',id:'a',revision:server.revision(current.tasks[0]),changes:{comment:'Nota de contexto\n'+'x'.repeat(4000)}},'test');
 current=await server.snapshot(true);
 const withComment=current.tasks[0].fields;
 for(const key of Object.keys(beforeComment))if(key!==F.tasks.result)assert.deepEqual(withComment[key],beforeComment[key]);
 assert.match(withComment[F.tasks.result],/Comentario \(no implica cierre ni pago\)/);
 assert.match(m.board(current).tasks[0].history,/Nota de contexto/);
 const firstComment=withComment[F.tasks.result];
 await server.mutate({kind:'task',id:'a',revision:server.revision(current.tasks[0]),changes:{comment:'Segunda nota'}},'test');
 current=await server.snapshot(true);
 assert.ok(current.tasks[0].fields[F.tasks.result].startsWith(firstComment));
 await assert.rejects(server.mutate({kind:'task',id:'a',revision:server.revision(current.tasks[0]),changes:{comment:'x'.repeat(10001)}},'test'));
 await assert.rejects(server.mutate({kind:'task',id:'a',revision:server.revision(current.tasks[0]),changes:{stage:'done',comment:'Esto no es evidencia de cierre'}},'test'),/resultado/);
 count++;console.log('OK comentarios largos preservan historial y estado, se leen y no sustituyen evidencia');
