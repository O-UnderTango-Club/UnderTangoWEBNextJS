...
assert.deepEqual(groups.map(g=>g.name),['Brasil','Otro','Compartido','Acciones sin proyecto']);
...
assert.deepEqual(groups[0].history.map(t=>t.id),['cerrada']);
...
assert.ok(frontProjectGroups(later,next).find(g=>g.id==='Brasil').tasks.some(t=>t.id==='primera'&&t.rank===1));
assert.deepEqual(frontProjectGroups(data,data.fronts.find(f=>f.name==='Primario')),[]);
console.log('Project grouping passed: interleaved ranks, all actions, shared identity, history, unlinked, empty front and daily handover');
